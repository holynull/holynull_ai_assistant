            ...
            try:
                async for event in self._runnable.astream_events(
                    ...
                    if (
                        ...
                        def convert_to_serializable(event):
                            from langgraph.types import Send
                            if 'output' in event['data'] and isinstance(event['data']['output'], list) and all(isinstance(item, Send) for item in event['data']['output']):
                                data={'output':[{"node":item.node,'arg':item.arg} for item in event['data']['output']]}
                                # 将Send类型转换为可序列化的字典
                                return {
                                    'event': event['event'],
                                    'data': data # 或其他合适的序列化方式
                                }
                            return event
                        # print("@@@@@@@@@@@@@@@@@@@@@@")
                        # print(event)
